...
const AdminEditEvent = () => {
  ...
  const user = location.state?.user; // if needed
  ...
  useEffect(() => {
    if (!event) {
      alert("No event found. Redirecting to admin dashboard.");
      navigate("/admin");
    }
  }, [event, navigate]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (!token) {
        alert("You must be logged in as an admin to update an event.");
        return;
      }
      const response = await fetch(`http://localhost:5000/api/admin/events/${event._id}`, {
        method: "PUT",
        headers: {
